...
The only hook available is ~hass-entity-updated-hook~ which is called when the state of an entity is updated, regardless of if it changed or not.
is called.

#+BEGIN_SRC emacs-lisp :results none
(add-hook 'hass-entity-updated-hook (lambda () (message "An entity's state was updated.")))
#+END_SRC
